...
export function damcalc(linesCleared, isAllClear, isGarbageClear, isSpin, isMiniSpin) {
    ...
    if (linesCleared) {
        ...
        if (backToBackBonus > 0) {
            ...
            if (gameState.setoptions.b2bcharging && 
                gameState.stats.btb > gameState.setoptions.b2bcharge_at) {
                surge = Math.floor(
                    (gameState.stats.btb - gameState.setoptions.b2bcharge_at + gameState.setoptions.b2bcharge_base) * 
                    gameState.setoptions.garbagemultiplier
                );
            }
    ...
    switch (linesCleared) {
        ...
        case 1:
            if (isMiniSpin) {
                ...
                if (gameState.setoptions.zenith && 
                    !gameState.setoptions.zenith_expert && 
    ...
    if (linesCleared && gameState.stats.btb > 1) {
        ...
        if (gameState.setoptions.b2bchaining) {
            const b2bChainBonus = constants.garbage.BACKTOBACK_BONUS * (
                Math.floor(1 + Math.log1p(
                    (gameState.stats.btb - 1) * constants.garbage.BACKTOBACK_BONUS_LOG
                )) + (
                    gameState.stats.btb - 1 === 1 ? 0 : 
            ...
            if (gameState.setoptions.b2bextras && 
               (linesCleared === 4 || (linesCleared >= 2 && isSpin))) {
    ...
    if (gameState.stats.combo > 1) {
        ...
        if (gameState.setoptions.combotable === "multiplier") {
            ...
            if (gameState.stats.combo > 2) {
                garbageValue = Math.max(
                    Math.log1p(
                        constants.garbage.COMBO_MINIFIER * 
                        (gameState.stats.combo - 1) * 
                        constants.garbage.COMBO_MINIFIER_LOG
                    ), 
                    garbageValue
                );
            }
